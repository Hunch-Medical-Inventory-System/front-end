<template>
  ...
  const headers = ref([
    ...
    { title: 'Crew Member', key: 'crew_member_id', align: 'center', divider: true },
  ])
  ...
  const crewMemberId = parseInt(search.value, 10);
  ...
  if (!isNaN(crewMemberId)) {
    query.eq('crew_member_id', crewMemberId);
  }
