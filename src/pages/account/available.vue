...
<template>
  <v-app class="bg-container">
    <v-container class="rounded-lg">
      ...
      <v-alert v-if="alertMessage" border="top" class="mb-4 v_alert alert-pop-in" prominent type="warning">
        {{ alertMessage }}
      </v-alert>

      <v-data-table-server v-model:items-per-page="itemsPerPage" v-model:page="page" :headers="headers"
        item-value="name" :items="currentInventory.data" :items-length="currentInventoryLength"
        :loading="inventoryLoading" @update:options="loadInventory">
        <template v-slot:top>
        ...
        <template #item="{ item }">
          <tr>
            <td>{{ item.id }}</td>
            <td>{{ suppliesLoading ? 'Loading...' : currentSupplies.data.find(supply => supply.id ===
              item.supply_id)?.item }}</td>
            <td>
        ...
        <template #tfoot>
          <tr>
            <td colspan="9">
              <v-text-field v-model="search" class="ma-2" density="compact" hide-details
